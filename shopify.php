...
class ShopifyClient {
    ...
    public function getAuthorizeUrl($scope, $redirect_url = '', $access_mode) {
        //$url = "https://{$this->shop_domain}/admin/oauth/authorize?client_id={$this->api_key}&scope=" . urlencode($scope)."&grant_options[]=per-user";
        $url = "https://{$this->shop_domain}/admin/oauth/authorize?client_id={$this->api_key}&scope=" . urlencode($scope)."&grant_options[]=".urlencode($access_mode);
    ...
    public function validateSignature($query) {
        ...
        $signature = hash_hmac('sha256', $string, $this->secret, false);
        //$signature = $this->hex2bin(hash('sha256', $string, false));
        
        //echo 'String: '.$string.'  HMAC: '.$query['hmac'].'  Signature: '.$signature;die();
